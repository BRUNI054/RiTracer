...
public:
    ...
    camera(int pixel_nx, int pixel_ny, float viewport_height, float viewport_width, float focal_length, vec3 origin, vec3 dir) : pos(origin), focalLength(focal_length), imageplaneWidth(viewport_width), imageplaneHeight(viewport_height), nx(pixel_nx), ny(pixel_ny), W(dir/dir.length()) {
        vec3 b(0, 1.0f, 0);
