...
int main() {
    ...
    for (int i = 0; i < tris; i++) {
        ...
        polar point3 (2.5f, ((360.0f/tris)*(float(i)+(0.5f))), 19.0f);
        s.pushback_shape(std::make_shared<triangle>(point1.zAxisConversion(), point2.zAxisConversion(), point3.zAxisConversion(), std::make_shared<blinnPhong>(50.0f, yellow)));
    }
    s.pushback_shape(std::make_shared<sphere>(point(0.0f, 0.0f, -20.0f), 1.80f, std::make_shared<lambertian>(yellow)));
    s.pushback_shape(std::make_shared<triangle>(vec3(0.1, -0.2, 10.0), vec3(-0.7, -0.2, 10.0), vec3(-0.3, 0.2, 10.0), std::make_shared<blinnPhong>(400.0f, color(0.0, 0.0, 0.0))));
    s.pushback_shape(std::make_shared<triangle>(vec3(0.7, -0.2, 10.0), vec3(-0.1, -0.2, 10.0), vec3(0.3, 0.2, 10.0), std::make_shared<blinnPhong>(400.0f, color(0.0, 0.0, 0.0))));
